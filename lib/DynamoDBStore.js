...
export default class DynamoDBStore extends Store {
  ...
  constructor(options = {}, callback = DEFAULT_CALLBACK) {
    ...
    // AWS setup options
    if (
      !options.dynamoParams ||
      !options.dynamoParams.accessKeyId ||
      !options.dynamoParams.secretAccessKey ||
      !options.dynamoParams.region
    ) {
      throw new Error('Missing AWS parameters');
    }

    AWS.config.update(options.dynamoParams);
